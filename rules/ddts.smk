...
rule map_reads_to_ref:
    ...
    shell:
        """
        minimap2 -t {threads} -ax {params.preset} {params.ref_to_use} {params.fastq} | samtools sort -o {output.bam}
        """
...
rule extract_inserts:
    ...
    shell:
        ...
        python somrit.py extract --bam {params.base_dir}/{input.bam} --output-tsv {params.base_dir}/{output.tsv} --output-merged {params.base_dir}/{output.merged_reads} --fastq-file {params.fastq} --threads {threads} --min-read-len 8000
        cd {params.base_dir}
        """
...
rule minigraph_align_augmented_graph:
    input:
...
rule graph_align_augmented_graph:
    input:
        "combined_normals/augmented_graph/combined.gfa",
    output:
        "{sample}/graph_aligner/{sample}.gaf"
    threads: 10
    params:
        memory_per_thread="6G",
        fastq=get_fastq
    shell:
        """
        GraphAligner -x dbp -t {threads} -g {input} -f {params.fastq} -a {output} 
        """
...
rule combine_sniffles_snf:
    input:
        expand("{sample}/sniffles/{sample}.sniffles.snf",sample=config["normals"])
    output:
    ...
    shell:
        """
        sniffles -t {threads} --input {input} --vcf {output}
        """
...
rule align_hg38_graph:
    output:
        "{sample}/hg38_graph/{sample}.gaf"
    threads: 10
    params:
        memory_per_thread="6G",
        fastq=get_fastq
    shell:
        """
        {config[minigraph]} -c -x lr -t {threads} {config[hg38_graph]} {params.fastq} > {output}
        """
...
rule call_translocations_augmented_graph:
    ...
    shell:
        """
        python {params.script} --gaf {input.gaf} --gfa {input.gfa} --nodes {input.nodes} --fastq {config[sample_data_folder]}/{wildcards.sample}/fastq/{wildcards.sample}.fastq.gz --ref {params.ref} --racon /u/adcosta/SimpsonLab/racon/build/bin/racon> {output}
        """
...
rule normalize_rgfa_graph:
    input:
        gaf="{sample}/augmented_graph_rgfa/{sample}.gaf",
        gfa="combined_normals/augmented_graph_rgfa/combined.gfa"
    output:
        norm="{sample}/augmented_graph_rgfa/{sample}.normalized.tsv",
        norm_hap="{sample}/augmented_graph_rgfa/{sample}.norm_hap.tsv",
        inserts="{sample}/augmented_graph_rgfa/{sample}.inserts.tsv"
    threads: 1
    params:
        memory_per_thread="96G",                                                                                                                                        script=srcdir("../scripts/identify_spanning_inserts.py"),
    shell:
        """
        python {params.script} --gaf {input.gaf} --gfa {input.gfa} --tsv {config[ddts_tsv]} --sample {wildcards.sample} --normalized {output.norm} --normalized-hap {output.norm_hap} --hap1 {config[ddts_data]}{wildcards.sample}/polymorphic/{wildcards.sample}_inserts.hap1.bam --hap2 {config[ddts_data]}{wildcards.sample}/polymorphic/{wildcards.sample}_inserts.hap2.bam > {output.inserts}
        """
